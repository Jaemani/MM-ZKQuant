...
> **2026-09-24: 공개 Monad 테스트넷 ZK 승인·거절 검사도 통과.** 정상 증명 승인, 목표 수량 변조 거절, 재사용 거절을 실제 트랜잭션 3건으로 확인했습니다(계약 배포 2건 별도). **이 공개 검사는 합성 입력이며 TEE·Perpl 체결을 포함하지 않습니다.** [팀 전달용 최신 판정과 트랜잭션](docs/2026-09-24-core-validation-handoff.md)을 기준으로 확인하세요.
...
| 공개 테스트넷 ZK | 정상 승인 1건, 변조·재사용 거절 각 1건, 실제 영수증·계약 상태 확인 | 합성 입력의 독립 probe만 통과. 공개 TEE→Perpl 전체 경로 미검증 |
...
추가로 [단일 트랜잭션 검사](docs/core-mvp.md#단일-트랜잭션-추가-검증) 32개를 통과했습니다. 성공 주문 6건에서 증명 검증→Perpl 실행의 내부 호출을 추적했고, Perpl에서 실패하면 nonce·포지션·담보가 함께 원복됨을 확인했습니다. [팀 검토 시나리오](docs/core-mvp-scenarios.md)에 보여줄 데이터와 합격 기준을 정리했습니다.

## 공개 테스트넷 진행 상태 — 2026-09-24
