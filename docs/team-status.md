...
- 공식 Perpl ABI·배포 버전과 실제 Monad RPC를 조사했다. 거래나 입출금은 실행하지 않았다.
...
| 단계 | 상태 | 종료에 남은 것 |
|---|---|---|
| M0 | 조사 진행, 미종료 | B1 실행·귀속, B2 지급 지연, B3 private 경로. B4는 기본 호환만 확인 |
...
| M7–M8 | 미진행 | 공개 테스트넷·privacy red team·외부 감사·setup·법률 검토 |
...
이번 작업에 유료 인프라 생성·실제 주문·자금 이동은 없다. 이전 Phala VM은 전원 종료만 확인됐고 디스크 삭제는 미확인이다. 현재 잔액을 과거 관측값으로 단정하지 않으며 승인된 $2 첫 실험 예산을 늘리지 않는다.
