> 보존 기록: 2026-09-20 이전 README입니다. 현재 기준은 [루트 README](../../README.md)와 [팀 개발명세](../team-development.md)입니다.
...
- [Concept Specification v0.2](../confidential-alpha-v0.2.md)
- [개발 구조, 검증 결과, 실행 방법과 남은 범위](../sleeves-v0.2.md)
- [테스트넷 실행 증거](../evidence/omnibus-testnet.json) · [RPC 재검증](../evidence/omnibus-rpc-verification.json)
...
**로컬 구현 검증과 공개 테스트넷 최종 인수는 구분합니다.** 로컬 EVM에서는 실제 ERC-20 잔고·대차·AMM·지급이 이동하고, 가격 원자료는 Kraken에서 수집합니다. 외부 Monad 거래와 독립 운영자 3명의 참여가 확인돼야 최종 gate를 통과합니다. 최신 숫자·잔액·판정은 [자동 재현 보고서](../evidence/pilot-readiness.md)를 보세요.

- [실행·가스 준비·제공자 온보딩·중단 복구](../pilot/runbook.md)
- [개발명세·정책·API·담당 경계](../pilot/development-spec.md)
- [원본 범위와 이전 프로토타입의 문제](../mvp-readiness.md)
...
| 문서 | 용도 |
|---|---|
| [요구사항](../requirements.md) | 제품 범위, R01–R20, 보장과 보장하지 않는 것 |
| [개발명세](../development-spec.md) | 객체·상태·API·서명/hash 계약·평가 공식·회계 |
| [5명 작업 분배 및 실행 계획](../three-week-execution-plan.md) | A–E 역할, ticket별 선행 작업·산출물·완료 조건, 후속 단계 |
| [인수 시험](../acceptance-tests.md) | 정상·실패·변조·시간 경계·재시작·화면 검증 기준 |
| [검증 기록](../mvp-validation.md) | 이번 구현에서 실제 수행한 검증과 남은 범위 |
| [MVP 준비도 판정](../mvp-readiness.md) | 원본 제품 범위와 현재 증거 비교, 미충족 gate 및 재현 방법 |
...
이전 TEE 전략 실행 문서는 [archive/legacy-tee](../archive/legacy-tee/)에 원문을 보존했습니다. 현재 요구사항 문서는 프로토타입 구현 계약이며 원본에서 합의한 alpha fund 범위보다 우선하지 않습니다. 기존 `research/`는 과거 연구 근거로 남아 있으며 현재 구현의 기능 목록은 아닙니다.
