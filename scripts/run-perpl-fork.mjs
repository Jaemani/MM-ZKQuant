...
const anvil=spawn(process.execPath,[wrapper,'--fork-url',originUrl,'--fork-block-number',block,'--chain-id',testnetRehearsal?'10143':'31337','--host','127.0.0.1','--port',String(port),'--silent'],{stdio:['ignore','ignore','pipe']});
...
try {
  ...
  const child=spawn(process.execPath,testnetRehearsal?['scripts/prove-perpl-testnet.mjs','--fork-rehearsal']:['scripts/prove-perpl-fork.mjs',...process.argv.slice(2).filter(x=>['--zk','--tee','--atomic'].includes(x))],{stdio:'inherit',env:{...process.env,MM_PERPL_FORK_RPC:rpcUrl}});
